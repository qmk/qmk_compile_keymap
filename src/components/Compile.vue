<template>
  ...
  <v-dialog v-model="dialog" width="auto" persistent @after-enter="submitJob">
    <v-card max-width="400" title="Compiling...">
      <template #text>
...
const { pause, resume } = useIntervalFn(async () => {
    const { data } = await useFetch(`${COMPILE_BASE_URL}/${jobID.value}`).get().json();
...
const submitJob = async () => {
    const { data } = await useFetch(COMPILE_BASE_URL).post(keymap.value).json();
